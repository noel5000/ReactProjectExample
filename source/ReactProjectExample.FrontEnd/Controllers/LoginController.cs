...
namespace ReactProjectExample.FrontEnd.Controllers
{
    ...
    public class LoginController : ControllerBase
    {
        ...
        public LoginController(IMemoryCache cache, IOptions<AppSettings> appSettings, IDataRepositoriesFactory repositoryFactory) 
        {
        ...
        public  IActionResult Post([FromBody] Login model)
        {
            try
            {
                User user = users.Get(x => x.Where(u => u.DeletedFlag == false && u.Email == model.Email && u.Password == MD5.Encrypt(model.Password, _appSettings.Value.TokenKey)));
                if (user == null)
